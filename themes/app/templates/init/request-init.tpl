...
<script src="{$THEME_URL}/js/maskedinput.js"></script>
<script src="{$THEME_URL}/js/bootbox/bootbox.min.js"></script>
<script>
    ...
    var Request = function() {
        ...
        var _subscribeConfirmResendForm = function () {
            ...
            resendButton.click(function () {
                ...
                /*$.post('{$GLOBAL_URL}/subscription/resend.php', subscribeConfirmForm.serialize()).done(function (data) {
                    if (data.success === true) {
                        $('#subscription-verify #code').val('');
                    } else {
                        $('#errors-verify span').remove();
            ...
            newsletterForm.validate({
                ...
                submitHandler: function () {
                    $.post(newsletterForm.attr("action"), newsletterForm.serialize()).done(function (data) {
                        ...
                        if (data.code === true) {
                            bootbox.alert({
                                title: "{$LANG.newsletter_success}",
                                message: '<h4>'+data.info+'</h4>',
                            });
                        }
                        if (data.code === false) {
                            bootbox.alert({
                                title: "{$LANG.newsletter_error}",
                                message: '<h4>'+data.info+'</h4>',
                            });
