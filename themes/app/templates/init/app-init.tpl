...
var App = function() {
    ...
    var _cartAction = function () {
        ...
        BtnAddToCart.on('click', function() {
            ...
            console.log(amount);
            $.post('/public/app.php', { id: id, act: action, amount: amount, lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
               console.log(data)
                _amountProductsCart();
            });
        });
        ...
        .on("keypress", function( e ) {
            if (e.keyCode === 13) {
                ...
                $.post('/public/app.php', { id: id, amount: amount, act: action, lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
                   // console.log(data)
                    if (data.code === true){
        ...
        $(".btn-remove").on('click', function() {
            ...
            $.post('/public/app.php', { id: $(this).attr("data-id"), act: $(this).attr('data-action'), lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
               // console.log(data);
                if (data.code === true) {
                    _amountProductsCart();
                    _totalPriceCart();
        ...
        $('.remove-all-products').on('click', function() {
            $.post('/public/app.php', { act: $(this).attr('data-action'), lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
               if (data.code === true) {
    ...
    var _amountProductsCart = function () {
        $.post('/public/app.php', { act: 'cart_products_amount', lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
    ...
    var _totalPriceCart = function () {
        $.post('/public/app.php', { act: 'cart_total_price', lang: '{if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}' }, function(data) {
    ...
    var _orderForm = function () {
        ...
        orderForm.validate({
            ...
            submitHandler: function () {
                $.post(orderForm.attr("action"), orderForm.serialize()+"&lang={if $SITE_LANG eq ''}ru{else}{$SITE_LANG}{/if}").done(function (data) {
                    console.log(data)
                    if (data.code === true) {
                        orderForm.fadeOut();
