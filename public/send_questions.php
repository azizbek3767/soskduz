...
$user_city = filter_input(INPUT_POST, 'city', FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_LOW);
...
if ($res = App::isValidCsrfToken($token)) {
    ...
    if ($feedback->setCaptchaEnabled() == 1) {
        ...
        if ($feedback->setAllowCaptcha() == 1) {
            ...
            if (empty($feedback->errors) && $feedback->captchaToken($_POST['g-recaptcha-response'])) {
    ...
    if (empty($feedback->errors) && $captcha_good == true) {
        ...
        if (empty($feedback->errors) && $feedback->sendFeedbackSMTP($user_name, $htmlBody, $user_email, $file)) {

            $success = array('code' => true, 'name' => $user_name);
