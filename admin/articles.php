<?php
	...
	$type_content  = getRequestVar('type_content', 'articles');
    $table = 'articles';
	...
	}elseif (!empty($action['deleteFileContent']) && !empty($article['articleId'])) { // удаление картинки
		$where[] = "articleId='$article[articleId]'";
       

        if ($file = dbQuery('article_files', DB_ARRAY, array('where' => $where))) {
            dbQuery('article_files', DB_DELETE, array('where'=> "fileId='$file[fileId]'" ));
			unlink(SITE_ROOT."/uploads/articles/$article[articleId]/$file[fileName]");
			dbQuery('articles', DB_UPDATE, array('where' => $where, 'values'=>"hasFile=0"));
			...
			echo "writeStatus('".lang('docs:errors:5')."', 'aError');\r\n";
			echo "window.setTimeout(\"writeStatus('')\", 5000);\r\n";
			exit;
		}
