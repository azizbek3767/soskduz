<?php
	if(empty($dsCookieName)) $dsCookieName = 'chartDateSelector';
	...
	if(!empty($simplePeriod)){
		switch ($simplePeriod){
			...
			case 'last14days':
				...
				$dateStart = gmdate('Y-m-d', strtotime("-2 weeks $config[hour_adjustment] hours $config[minute_adjustment] minutes"));
			break;
			case 'last30days':
				$dateEnd = gmdate('Y-m-d', strtotime("$config[hour_adjustment] hours $config[minute_adjustment] minutes"));
		...
		$simplePeriods = array(
			...
			'last14days'=>lang('stats:periods:last14days'),
			'last30days'=>lang('stats:periods:last30days'),
