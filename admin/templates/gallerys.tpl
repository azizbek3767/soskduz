...
<div class="page-content-wrap">
    {if isset($messages.saved)}<span id="savedMessage" onclick="noty({ text: '{gallery:messages:0}', layout: 'topRight', type: 'success', timeout: 1500 });"></span>{/if}
    {if isset($errors.access_denied)}<span id="accessDeniedError" onclick="noty({ text: '{gallery:errors:0}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.title)}<span id="titleError" onclick="noty({ text: '{gallery:errors:1}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.sectionId)}<span id="sectionIdError" onclick="noty({ text: '{gallery:errors:2}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.not_saved)}<span id="notSavedError" onclick="noty({ text: '{gallery:errors:3}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.no_sections)}<span id="noSectionsError" onclick="noty({ text: '{gallery:errors:4}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.image_not_saved)}<span id="imageNotSavedError" onclick="noty({ text: '{gallery:errors:5}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.article_not_found)}<span id="articleNotFoundError" onclick="noty({ text: '{gallery:errors:6}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileName)}<span id="fileNameError" onclick="noty({ text: '{gallery:errors:7}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.fileNameExists)}<span id="fileNameExistsError" onclick="noty({ text: '{gallery:errors:8}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.htaccess)}<span id="htaccessError" onclick="noty({ text: '{gallery:errors:9}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.serializations)}<span id="serializationsError" onclick="noty({ text: '{gallery:errors:10}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileNameProhibited)}<span id="fileNameProhibitedError" onclick="noty({ text: '{gallery:errors:11}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileNameCharacters)}<span id="fileNameCharactersError" onclick="noty({ text: '{gallery:errors:12}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	<script>
        ...
        function deleteImageMessage(){ noty({ text: '{gallery:messages:1}', layout: 'topRight', type: 'success', timeout: 1500 }) }
        function deleteGalleryMessage(){ noty({ text: '{gallery:messages:2}', layout: 'topRight', type: 'success', timeout: 1500 }) }
...
{if isset($action.edit)}
	...
	<form action="gallerys.php" method="post" id="article" enctype="multipart/form-data">
                ...
                <div class="panel-body tab-content nopadding">
                            ...
                            <div class="panel-body">  
                                <div class="col-md-8"> 
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.sectionId)}fError{/if}">{gallery:general:section}</label>
                                        {if isset($article.sectionId)}
                                            {html_options options=$sections name="article[sectionId]" id="sectionId" selected=$article.sectionId|default:"0" class="form-control select"}
                                        {else}
                                            {html_options options=$sections name="article[sectionId]" id="sectionId" class="form-control select"}
                                        {/if}
                                    </div>
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.title)}fError{/if}">{gallery:general:title} </label>
                                    ...
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.fileName) || isset($errors.fileNameExists) || isset($errors.fileNameProhibited) || isset($errors.fileNameCharacters)}fError{/if}">{gallery:general:filename}</label>
                                    ...
                                    <div class="form-group">
                                        <label class="field_name">{gallery:general:alias}</label>
                                ...
                                <div class="col-md-4"> 
                                    <div class="form-group">
                                        <div class="col-md-12 nopadding">
                                            <label class="field_name">{gallery:general:primaryImage}</label>
                                            ...
                                            <div class="file-previewzo-obzor" id="imageOptions">
                                                ...
                                                <div class="file-preview-thumbnails">
                                                    <div class="file-preview-frame" id="imageOptions">
                                                        <img src="{$article.images.medium.url}" class="file-preview-image" alt="{$article.images.original.fileName}">
                                                    </div>
                    ...
                    <div class="tab-pane" id="gallery">
                        <div class="panel-body">  
                            ...
                            <div class="col-md-4 col-xs-12">
                                <div class="col-md-12 col-xs-12 nopadding"><div class=" push-up-10  push-down-10 control-label"><label class="field_name">{sections:addToGallery}</label></div></div>
                    ...
                    <div class="tab-pane" id="misc">
                        <div class="panel panel-default">
                            <div class="panel-body"> 
                                <div class="form-group">
                                    <div class="col-md-6 nopadding_left"><label class="field_name">{gallery:misc:publishDate}</label></div>
                                ...
                                <div class="form-group">
                                    <div class="col-md-6 nopadding_left"><label class="field_name">{gallery:general:status}</label></div>
                                ...
                                <div class="form-group">
                                    <div class="col-md-6 nopadding_left"><label class="field_name">{gallery:misc:featured}</label></div>
                                    <div class="col-md-6 nopadding_right">
                                ...
                                <div class="form-group">
                                    <div class="col-md-6 nopadding_left"><label class="field_name">{gallery:misc:comments}</label></div>
                                    <div class="col-md-6 nopadding_right">
        ...
        <div class="col-md-12 main main_buttons text-center">
            <input class="btn btn-primary" type="submit" name="action[save]" value="&nbsp; {general:save} &nbsp;" onclick="document.getElementById('article').target='_self'; document.getElementById('article').action='{$GLOBAL_URL}/admin{if $SITE_LANG != ''}/{$SITE_LANG}{/if}/gallerys.php{if isset($page)}?page={$page}{/if}'" />	&nbsp;
            <a class="btn btn-primary" href="gallerys.php{if isset($page)}?page={$page}{/if}">{general:cancel}</a>
        </div>
    </form>
...
{elseif !isset($errors.access_denied)}
    ...
    <div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top">                        
            <div class="col-md-3 col-xs-3 nopadding_left"><h2>{gallery:title}</h2></div> 
            <div class="col-md-6 col-xs-7 nopadding_left">
    	        <form id="searchForm">
    	    	    <div class="col-md-12 col-xs-12 nopadding_left">
        	    	    <input id="quick-search" class="form-control" type="text" name="query" value="{if isset($query)}{$query}{/if}" autocomplete="off" style="padding-right: 75px;"/>
        	    	    <input style="position: absolute;top: 0;right: 0;"class=" btn btn-danger" type="submit" value="{general:search}"/>
    	    	    </div>
    		    </form>
    	    </div> 
            <div class="col-md-3 col-xs-2 nopadding_right">                                                
                                ...
                                {foreach item=article from=$articles name=articles}
                                    <tr id="article-{$article.articleId}">
                                        ...
                                        <td class="btn-link-action text-center">
                                            <a alt="{general:edit}" class="btn btn-rounded" href="gallerys.php?action[edit]=true&article[articleId]={$article.articleId}{if isset($page)}&page={$page}{/if}"><span class="fa fa-pen"></span></a>
