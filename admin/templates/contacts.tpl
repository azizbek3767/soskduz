{include file="header.tpl" activeItem="contacts" title="{contacts:title}"}
<div class="page-content-wrap">

    {if isset($messages.saved)}<span id="savedMessage" onclick="noty({ text: '{contacts:messages:0}', layout: 'topRight', type: 'success', timeout: 1500 });"></span>{/if}
    {if isset($errors.access_denied)}<span id="accessDeniedError" onclick="noty({ text: '{contacts:errors:0}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.title)}<span id="titleError" onclick="noty({ text: '{contacts:errors:1}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.sectionId)}<span id="sectionIdError" onclick="noty({ text: '{contacts:errors:2}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.not_saved)}<span id="notSavedError" onclick="noty({ text: '{contacts:errors:3}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.no_sections)}<span id="noSectionsError" onclick="noty({ text: '{contacts:errors:4}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.image_not_saved)}<span id="imageNotSavedError" onclick="noty({ text: '{contacts:errors:5}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.article_not_found)}<span id="articleNotFoundError" onclick="noty({ text: '{contacts:errors:6}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileName)}<span id="fileNameError" onclick="noty({ text: '{contacts:errors:7}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if} 
	{if isset($errors.fileNameExists)}<span id="fileNameExistsError" onclick="noty({ text: '{contacts:errors:8}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.htaccess)}<span id="htaccessError" onclick="noty({ text: '{contacts:errors:9}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.serializations)}<span id="serializationsError" onclick="noty({ text: '{contacts:errors:10}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileNameProhibited)}<span id="fileNameProhibitedError" onclick="noty({ text: '{contacts:errors:11}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	{if isset($errors.fileNameCharacters)}<span id="fileNameCharactersError" onclick="noty({ text: '{contacts:errors:12}', layout: 'topRight', type: 'error', timeout: 1500 });"></span>{/if}
	<script>
    ...
    function deleteImageMessage(){ noty({ text: '{contacts:messages:1}', layout: 'topRight', type: 'success', timeout: 1500 }) }
    function deleteArticleMessage(){ noty({ text: '{contacts:messages:2}', layout: 'topRight', type: 'success', timeout: 1500 }) }
...
{if isset($action.edit)}
	<div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top">
            <div class="col-md-3 col-xs-3 nopadding_left"><h2>{contacts:title}</h2></div>
	...
	<form action="contacts.php" method="post" id="article" enctype="multipart/form-data">
        ...
        <div class="col-md-12">
            <div class="panel panel-default tabs">
                <ul class="nav nav-tabs nav-justified">
                    <li class="active"><a href="#general" data-toggle="tab" aria-expanded="true">{contacts:tabs:general}</a></li>
                    <li><a href="#summary" data-toggle="tab" aria-expanded="false">{contacts:tabs:summary}</a></li>
                    <li class=""><a href="#other" data-toggle="tab" aria-expanded="false">{contacts:tabs:misc}</a></li>
                    <li class=""><a href="#seo" data-toggle="tab" aria-expanded="false">{contacts:tabs:seo}</a></li>
                </ul>
    		    <div class="panel-body tab-content nopadding">
                            ...
                            <div class="panel-body">
                                <div class="col-md-8">
                                    ...
                                    <div class="form-group">
                                        <label class="field_name">{contacts:status}</label>
                                    ...
                                    {/if}
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.sectionId)}fError{/if}">{contacts:general:section}</label>
                                        {if isset($sections)}
                                            ...
                                            {else}
                                             ...
                                             {/if}
                                        {else}
                                            {contacts:general:notAvailable}<input type="hidden" name="article[sectionId]" value="0"/>
                                        {/if}
                                    </div>
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.title)}fError{/if}">{contacts:general:title}</label>
                                    ...
                                    <div class="form-group">
                                        <label class="field_name {if isset($errors.fileName) || isset($errors.fileNameExists) || isset($errors.fileNameProhibited) || isset($errors.fileNameCharacters)}fError{/if}">{contacts:general:filename}</label>
                                    ...
                                    <div class="form-group">
                                        <label class="field_name">Координаты</label>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <div class="col-md-12 nopadding">
                                            <label class="field_name">{contacts:general:primaryImage}</label>
                    ...
                    <div class="tab-pane" id="other">
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <div class="form-group">
                                    <div class="col-md-6 nopadding_left"><label class="field_name">{contacts:misc:publishDate}</label></div>
        ...
        <div class="col-md-12 main main_buttons text-center">
		    <input class="btn btn-primary" type="submit" name="action[save]" value="&nbsp; {general:save} &nbsp;" onclick="document.getElementById('article').target='_self'; document.getElementById('article').action='{$GLOBAL_URL}/admin{if $SITE_LANG != ''}/{$SITE_LANG}{/if}/contacts.php{if isset($page)}?page={$page}{/if}'" /> &nbsp;
            <a class="btn btn-primary" href="contacts.php{if isset($page)}?page={$page}{/if}">{general:cancel}</a>
        </div>
    </form>
...
{elseif !isset($errors.access_denied)}
    ...
    <div class="content-frame" style="margin-bottom:10px;">
        <div class="content-frame-top">                        
            <div class="col-md-6 col-xs-6 nopadding_left"><h2>{contacts:title}</h2></div> 
            <div class="col-md-6 col-xs-6 nopadding_right">                                                
                <div class="pull-right"> 
                    <a class="btn btn-danger" href="contacts.php?action[edit]=true" data-toggle="tooltip" data-placement="left" data-original-title="{contacts:addcontacts}"><i class="fa fa-plus"></i></a>
                            ...
                            <tr id="article-{$article.articleId}">
                                ...
                                <td style="line-height: 58px;min-width: 120px;" id="status-{$article.articleId}">
                                    ...
                                    {if $adminUser.accessLevel != 'writer' && $article.status eq 'pending'}<br />
                                        <p class="act-deistvie action small" onclick="return approveContent({$article.articleId}, 'contacts');">({contacts:approve})</p>
                                    {/if}
                                </td>
                                <td class="btn-link-action text-center">
                                    <a class="btn btn-rounded" href="contacts.php?action[edit]=true&article[articleId]={$article.articleId}&page={$page}"><span class="fa fa-pen"></span></a>  
                                    <a class="btn btn-danger btn-rounded" onclick="deleteContent({$article.articleId}, '{$article.title nofilter}', 'contacts');"><span class="fa fa-trash"></span></a>
                                </td>
                            </tr>
  						{/foreach}
                        </tbody>
                            ...
                            <tr>
                                ...
                                <td align="right" colspan="3">
                                    ...
                                    <ul class="pagination pagination-sm pull-right push-down-10 push-up-10">
                                        ...
                                        {if $number eq $page}
                                        <li class="active"><a href="contacts.php?page={$number}{if isset($query)}&query={$query}{/if}{if isset($section)}&section={$section}{/if}{if isset($status)}&status={$status}{/if}">{$number}</a></li>
                                        {elseif $number eq '...'}
                                        ...
                                        {else}
                                        <li><a href="contacts.php?page={$number}{if isset($query)}&query={$query}{/if}{if isset($section)}&section={$section}{/if}{if isset($status)}&status={$status}{/if}">{$number}</a></li>
                                        {/if}
                                        {/foreach}
                                    </ul>  
                                    {/if}
...
<script>
    ...
    $('.save_edit_image').click(function(e){	
        ...
        $.post('contacts.php', { id: id, desc: desc, link: link, action: 'editimage' }, function(data){
            if (data.update == 1){            
